...
<div id="impersonate-ui" class="position-{{ config('laravel-impersonate-ui.icon_position') }}">
	...
	<div id="impersonate-ui-fullcontainer" class="hiddenX">
		<h3 class="h5">Impersonate a User</h3>
		Impersonated user: <span id="current-user"><strong>{{ Auth::user()->name }}</strong></span><br>
		@impersonating
			Actual user: <span id="real-user"><strong>{{ $impersonator->name }}</strong></span>
		@endImpersonating
			...
			<div class="form-group mb-1 mt-2">
				...
				<select name="impersonate_id" id="impersonate_id" class="custom-select" {!! !config('laravel-impersonate-ui.show_button') ? ' onchange="javascript:this.form.submit()"' : '' !!}>
					@foreach($users as $user)
						...
						@else
							<option value="{{ $user->id }}" selected disabled>{{ $user->name }} &lt;{{ $user->email }}&gt;</option>
...
<script type="text/javascript">
	function toggleFull(){
		let fullcontainer = document.querySelector('#impersonate-ui-fullcontainer');
